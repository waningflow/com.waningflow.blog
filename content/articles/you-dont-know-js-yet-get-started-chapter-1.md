...
所有 TC39 提案都要经过"5-stage"流程--当然，因为我们是程序员，所以它是从 0 开始计数的！--"Stage 0"到"Stage 4"。您可以在此处阅读有关 Stage 流程的更多信息：https://tc39.es/process-document/。
...
所有提案都在 TC39 的 Github 中公开管理：https://github.com/tc39/proposals/。
...
程序离开开发人员的编辑器后，由 Babel 进行编译，然后由 Webpack 打包（可能一堆构建过程），然后以完全不同的形式交付给 JS 引擎。
JS 引擎将代码转成 AST。
然后引擎将 AST 转成某种字节码，一种二进制的中间形态（intermediate representation (IR)），然后通过优化 JIT 编译器进一步完善/转化。
最后，JS 虚拟机运行程序。
再看一遍这些步骤：
